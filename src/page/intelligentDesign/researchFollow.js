...
class ResearchFollow extends PureComponent {
  ...
  handleOptionsFourth=async (values,optionTitle,stepName,modelName)=>{
    ...
    let subObj={
      ...
      stepName,modelName};
    if(optionTitle ==='衣长'){
      subObj.yichang=values;
    } else if(optionTitle ==='肩部'){
      subObj.jianbu=values;
    } else if(optionTitle ==='腰头'){
      subObj.yaotou=values;
    } else if(optionTitle ==='底摆'){
      subObj.dibai=values;
    }
    let flag = false;
    if(newKxsjDesginArr.length>0){
      flag = newKxsjDesginArr.some(item=>{return item.picCheck === true});
    }
    if(flag){
      ...
      if(flag){
        for (let i = 0; i < newSubmitData.length; i++) {
          if (newSubmitData[i].modelName === subObj.modelName
            ...
            if(optionTitle ==='衣长'){
              newSubmitData[i].yichang=values;
            } else if(optionTitle ==='肩部'){
              newSubmitData[i].jianbu=values;
            } else if(optionTitle ==='腰头'){
              newSubmitData[i].yaotou=values;
            } else if(optionTitle ==='底摆'){
              newSubmitData[i].dibai=values;
            }
          }
        }
      } else {
  ...
  handleOptionsFifth=async (values,optionTitle,stepName,modelName)=>{
    ...
    if(newJgxsjDesginArr.length>0){
      flag = newJgxsjDesginArr.some(item=>{return item.picCheck === true});
    }
    if(flag){
      for(let i=0;i<newJgxsjDesginArr.length;i++){
        let {theme_name,series_name,dress_style,silhouette,craft,mountings,style,name,picCheck,yichang,jianbu,yaotou,dibai} = newJgxsjDesginArr[i];
        if(picCheck){
          subObj.theme_name=theme_name||'';
          subObj.series_name=series_name||'';
          subObj.dress_style=dress_style||'';
          ...
          subObj.style=style||'';
          subObj.name=name||'';
          subObj.yichang=yichang||'';
          subObj.jianbu=jianbu||'';
          subObj.yaotou=yaotou||'';
          subObj.dibai=dibai||'';
        }
      }
    } else {
      newJgxsjArr.forEach(item=>{
        ...
        dressStyleArr.forEach(cur=>{
          let {designData} = cur;
          designData.forEach(c=>{
            let {theme_name,series_name,dress_style,silhouette,craft,mountings,style,picCheck,yichang,jianbu,yaotou,dibai} = c;
            if(picCheck){
              subObj.theme_name=theme_name||'';
              subObj.series_name=series_name||'';
              subObj.dress_style=dress_style||'';
  ...
  handleOptionsSixth=async (values,optionTitle,stepName,modelName)=>{
    ...
    if(newXjsjDesginArr.length>0){
      flag = newXjsjDesginArr.some(item=>{return item.picCheck === true});
    }
    if(flag){
      for(let i=0;i<newXjsjDesginArr.length;i++){
        let {theme_name,series_name,dress_style,silhouette,craft,mountings,style,name,picCheck,yichang,jianbu,yaotou,dibai,kaijin,lingzi,koudai,koudai_len} = newXjsjDesginArr[i];
        if(picCheck){
          subObj.theme_name=theme_name||'';
          subObj.series_name=series_name||'';
          subObj.dress_style=dress_style||'';
      ...
      newXjsjArr.forEach(item=>{
        ...
        dressStyleArr.forEach(cur=>{
          let {designData} = cur;
          designData.forEach(c=>{
            let {theme_name,series_name,dress_style,silhouette,craft,mountings,style,picCheck,yichang,jianbu,yaotou,dibai,kaijin,lingzi,koudai,koudai_len} = c;
            if(picCheck){
              subObj.theme_name=theme_name||'';
              subObj.series_name=series_name||'';
              subObj.dress_style=dress_style||'';
        ...
        if(typeof newOperateRecord[i].tempSubmitData!=='undefined'){
          if(newSubmitData.length>0){
            for(let s=0;s<newSubmitData.length;s++){
              let flag = tempSubmitData.some(item=>{return item.modelName === newSubmitData[s].modelName
  ...
  handleExecute=()=>{
    ...
    if(submitData.length>0){
      if(query.step ==='1'){
        ...
        let themeName="";
        themeName =submitData[0].theme_name;
        router.push(`/follow?modelName=${query.modelName ||''}&step=7&themeName=${themeName}`);
      }
      else if(query.step ==='7'){
    ...
    if(query.step ==='4'){
      if(parseInt(flag) ===1){
        ...
        if(submitData.length<=0){
          ...
          let flag = submitData.some(item=>{return item.silhouette === silhouette});
          if(flag){
            submitData.forEach(item=>{
              let {name} = item;
        ...
        if(newOperateRecord.length<=0){
          newOperateRecord.push({
            id:1, theme_name,series_name,dress_style,silhouette,craft,mountings,newPicCheck,newCardInfo,newCardActions
          })
        } else {
          newOperateRecord.sort((a,b)=>{return b.id - a.id});
          let id =newOperateRecord[0].id;
          newOperateRecord.push({
            id:id+1, theme_name,series_name,dress_style,silhouette,craft,mountings,newPicCheck,newCardInfo,newCardActions
          });
        }
        if(newPicCheck){
          if(newKxsjArr.length>0){
        ...
        let {theme_name,series_name,dress_style,silhouette,craft,mountings,name,yichang,jianbu,yaotou,dibai} = item;
        let newParameters = JSON.parse(JSON.stringify(parameters));
        for (let i = 0; i < newParameters.length; i++) {
          let {parameters} = newParameters[i];
          parameters.forEach(item=>{
            if(item.option_title ==='衣长'){
              item.values = yichang;
            } else if(item.option_title ==='肩部'){
              item.values = jianbu;
            } else if(item.option_title ==='腰头'){
              item.values = yaotou;
            } else if(item.option_title ==='底摆'){
              item.values = dibai;
            }
          })
        }
        ...
        if(newOperateRecord.length<=0){
          newOperateRecord.push({
            id:1, theme_name,series_name,dress_style,silhouette,craft,mountings,name,newPicCheck,newCardInfo,newCardActions
          })
        } else {
          newOperateRecord.sort((a,b)=>{return b.id - a.id});
          let id =newOperateRecord[0].id;
          newOperateRecord.push({
            id:id+1, theme_name,series_name,dress_style,silhouette,craft,mountings,name,newPicCheck,newCardInfo,newCardActions
          });
        }
        if(newPicCheck){
          if(newKxsjDesginArr.length>0){
          ...
          dressStyleArr.forEach((cur,index)=>{
            ...
            if(kxsjDesginArr.length>0){
              ...
              kxsjDesginArr.forEach(item=>{
                let flag= silhouetteArr.some(c=>{return c.silhouette === item.silhouette});
                if(!flag){
                  silhouetteArr.push({
              ...
              if(silhouetteArr.length>0){
                silhouetteArr.forEach(item=>{
                  let arr = kxsjDesginArr.filter(cur=>{return cur.silhouette === item.silhouette});
                  if(arr.length>0){
                    item.data = arr;
                  }
                });
                silhouetteArr.forEach(item=>{
                  ...
                  designDataVDOM1.push(
                    <div key={Math.random()} >
                      <Row type="flex"><Col style={{marginLeft: "40px",
                        marginTop: "115px",
                        color: "#ffffff",
                        fontSize: "15px"}}>{silhouette}</Col>{vDOM}</Row>
                    </div>
                  )
                });
              }
        ...
        if(dressStyleArr.length>0){
          dressStyleArr.forEach((cur,index)=>{
            ...
            if(jgxsjDesginArr.length>0){
              ...
              if(silhouetteArr.length>0){
                silhouetteArr.forEach(item=>{
                  let arr = jgxsjDesginArr.filter(cur=>{return cur.silhouette === item.silhouette});
                  if(arr.length>0){
                    item.data = arr;
                  }
                });
        ...
        if(dressStyleArr.length>0){
          dressStyleArr.forEach((cur,index)=>{
            ...
            if(xjsjDesginArr.length>0){
              ...
              if(silhouetteArr.length>0){
                silhouetteArr.forEach(item=>{
                  let arr = xjsjDesginArr.filter(cur=>{return cur.silhouette === item.silhouette});
                  if(arr.length>0){
                    item.data = arr;
                  }
                });
    ...
    return (
      <div className={style.designWrapper}>
        ...
        <div className={style.mainWrapper} style={{width: mainWidth, marginLeft: leftWidth}}>
          ...
          {
            this.state.checkStep ==='5' && this.state.jgxsjArr.length>0 &&
            this.renderJgxsjCommonDom()
          }
          {
            this.state.checkStep ==='6' && this.state.xjsjArr.length>0 &&
